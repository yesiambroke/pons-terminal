...
export async function dispatchTrade(
  ...
  if (market.kind === 'v1') return { ...(await injected.v1(wallet, client, p)), route: market.kind }
  if (market.kind === 'v2-curve') {
    return { ...(await injected.curve(wallet, client, { ...p, curve: market.curve })), route: market.kind }
  }
  const tokenIn = p.direction === 'buy' ? NATIVE_ETH : p.token
  const tokenOut = p.direction === 'buy' ? p.token : NATIVE_ETH
  const v4MinGrossOut = p.v4MinGrossOut ?? await injected.quoteV4(client, {
  ...
  return { ...(await injected.v4(wallet, client, { ...p, v4MinGrossOut, tickSpacing: market.tickSpacing })), route: market.kind }
}
